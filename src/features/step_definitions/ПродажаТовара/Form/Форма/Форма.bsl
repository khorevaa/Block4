...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ПроверяюСообщениеПользователю()","ПроверяюСообщениеПользователю","И Проверяю сообщение пользователю");
...
Процедура ПроверяюСообщениеПользователю() Экспорт
	ОжидаемаяСтрока = "По номенклатуре: Ручка шариковая на складе нет нужного количества, не хватает: -30";
	Нашел = Ложь;
	...
	Если Не Нашел Тогда
		ВызватьИсключение "Строка <" + ОжидаемаяСтрока + "> не найдена в окне сообщений пользователю.";
	КонецЕсли;	 
	Ванесса.Sleep(5);
...
Процедура ОкноПриложенияПродажаСозданиеКнопкаOKВыбратьСообщениеПользователю(ТестовоеПриложение)
	...
	КонтекстСохраняемый.ТестовоеПриложение.ПолучитьПодчиненныеОбъекты()[0].Закрыть();
