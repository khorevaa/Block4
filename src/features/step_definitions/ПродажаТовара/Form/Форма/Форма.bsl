...
Функция ПолучитьСписокТестов(КонтекстФреймворкаBDD) Экспорт
	...
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ПроверяюСообщениеПользователюИОноДолжноБытьПустым()","ПроверяюСообщениеПользователюИОноДолжноБытьПустым","И Проверяю сообщение пользователю и оно должно быть пустым");
	Ванесса.ДобавитьШагВМассивТестов(ВсеТесты,"ПроверяюСообщениеПользователю(Парам1)","ПроверяюСообщениеПользователю","И Проверяю сообщение пользователю");
...
&НаКлиенте
//И Проверяю сообщение пользователю и оно должно быть пустым
//@ПроверяюСообщениеПользователюИОноДолжноБытьПустым()
Процедура ПроверяюСообщениеПользователюИОноДолжноБытьПустым() Экспорт
	МассивСообщений = КонтекстСохраняемый.ТестовоеПриложение.ПолучитьАктивноеОкно().ПолучитьТекстыСообщенийПользователю();
	Если МассивСообщений.Количество() <> 0 Тогда
		ВызватьИсключение "Ожидали, что окна сообщений не будет";
	КонецЕсли; 
	
КонецПроцедуры

&НаКлиенте
//И Проверяю сообщение пользователю
//@ПроверяюСообщениеПользователю()
Процедура ПроверяюСообщениеПользователю(ОжидаемаяСтрока) Экспорт
	Нашел = НашелСообщениеПользователью(ОжидаемаяСтрока);
	Если НЕ Нашел Тогда
		ВызватьИсключение "Ожидал получить строку сообщений "+ОжидаемаяСтрока;
	КонецЕсли; 
	//Ванесса.Sleep(5);
...
&НаКлиенте
Функция НашелСообщениеПользователью(ОжидаемаяСтрока)

	Нашел = Ложь;
	МассивСообщений = КонтекстСохраняемый.ТестовоеПриложение.ПолучитьАктивноеОкно().ПолучитьТекстыСообщенийПользователю();
	Для каждого Сообщение Из МассивСообщений Цикл
		Если Найти(НРег(Сообщение),НРег(ОжидаемаяСтрока)) > 0 Тогда
			Нашел = Истина;
			Прервать;
		КонецЕсли;	 
	КонецЦикла;
